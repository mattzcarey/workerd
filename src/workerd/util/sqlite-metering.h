...
class SqliteMemoryScope {
 public:
  // memoryBytes: the per-actor byte counter owned by SqliteDatabase for its lifetime.
  // maxMemoryBytes: the per-actor cap from WorkerLimits::sqliteMaxMemoryMb.
  //
  ...
  SqliteMemoryScope(SqliteMemoryScope&& other);

  ~SqliteMemoryScope() noexcept(false);
  KJ_DISALLOW_COPY(SqliteMemoryScope);
